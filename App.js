...
export default function App() {
  ...
  const handleApplyBatch = async (batchId) => {
    ...
    for (var u of batch.updates) {
      if (u.type === 'paid') { paidCount++; var pSub = newSubs.find(function(s) { return s.id === u.subscriberId; }); if (pSub) { var pmParts = u.monthKey.split('_'); paidAmount += getAmperForMonth(pSub, parseInt(pmParts[0]), parseInt(pmParts[1])) * getPriceForSubscriber(amperPrices, goldenPrices, u.monthKey, pSub.subscriptionType); } }
      else if (u.type === 'cancelled') { cancelledCount++; }
      else if (u.type === 'partialPayment') { partialCount++; partialAmount += parseFloat(u.details && u.details.amount || 0); }
      else if (u.type === 'add') { addedCount++; }
      else if (u.type === 'delete') { deletedCount++; }
      else if (u.type === 'edit') { editedCount++; }
      else if (u.type === 'restore') { restoredCount++; }
      else if (u.type === 'addExpense') { expenseCount++; expenseAmount += parseFloat(u.details && u.details.amount || 0); if (u.details && u.details.expenseType) expenseTypes.push(u.details.expenseType); }
    }
    ...
    var waMsg = msgLines.join('\n');
    Alert.alert('إرسال تقرير', 'هل تريد إرسال تقرير التحديثات عبر الواتساب؟', [
      { text: 'تخطي', style: 'cancel' },
      { text: 'إرسال ✓', onPress: function() { var ownerPhone = currentUser; if (ownerPhone) { Linking.openURL('https://wa.me/' + ownerPhone + '?text=' + encodeURIComponent(waMsg)).catch(function() {}); } } }
    ]);
  };
